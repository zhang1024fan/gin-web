  ...
  methods: {
    ...
    async connectTerminal() {
      ...
      try {
        ...
        // 使用固定地址构造WebSocket URL
        const wsUrl = `ws://127.0.0.1:8000/api/v1/cmdb/hostssh/connect/${this.currentHost.id}?token=${encodeURIComponent(token)}`
        
        console.log('WebSocket连接URL:', wsUrl)
        this.term.writeln(`\r\n尝试连接到: ${wsUrl}`)
        this.term.writeln(`完整路径: ${wsUrl}`)
